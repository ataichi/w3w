
package Beans;

public class CustomerBean {
    private int apartmentNoBA;
    private String streetBA;
    private String subdivisionBA;
    private String cityBA;
    private int postalCodeBA;
    private String countryBA;
    private int apartmentNoDA;
    private String streetDA;
    private String subdivisionDA;
    private String cityDA;
    private int postalCodeDA;
    private String countryDA;

    public int getApartmentNoBA() {
        return apartmentNoBA;
    }

    public void setApartmentNoBA(int apartmentNoBA) {
        this.apartmentNoBA = apartmentNoBA;
    }

    public String getStreetBA() {
        return streetBA;
    }

    public void setStreetBA(String streetBA) {
        this.streetBA = streetBA;
    }

    public String getSubdivisionBA() {
        return subdivisionBA;
    }

    public void setSubdivisionBA(String subdivisionBA) {
        this.subdivisionBA = subdivisionBA;
    }

    public String getCityBA() {
        return cityBA;
    }

    public void setCityBA(String cityBA) {
        this.cityBA = cityBA;
    }

    public int getPostalCodeBA() {
        return postalCodeBA;
    }

    public void setPostalCodeBA(int postalCodeBA) {
        this.postalCodeBA = postalCodeBA;
    }

    public String getCountryBA() {
        return countryBA;
    }

    public void setCountryBA(String countryBA) {
        this.countryBA = countryBA;
    }

    public int getApartmentNoDA() {
        return apartmentNoDA;
    }

    public void setApartmentNoDA(int apartmentNoDA) {
        this.apartmentNoDA = apartmentNoDA;
    }

    public String getStreetDA() {
        return streetDA;
    }

    public void setStreetDA(String streetDA) {
        this.streetDA = streetDA;
    }

    public String getSubdivisionDA() {
        return subdivisionDA;
    }

    public void setSubdivisionDA(String subdivisionDA) {
        this.subdivisionDA = subdivisionDA;
    }

    public String getCityDA() {
        return cityDA;
    }

    public void setCityDA(String cityDA) {
        this.cityDA = cityDA;
    }

    public int getPostalCodeDA() {
        return postalCodeDA;
    }

    public void setPostalCodeDA(int postalCodeDA) {
        this.postalCodeDA = postalCodeDA;
    }

    public String getCountryDA() {
        return countryDA;
    }

    public void setCountryDA(String countryDA) {
        this.countryDA = countryDA;
    }
    
    
}
